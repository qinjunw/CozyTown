...
namespace CozyTown.Tests.EditMode.NpcAgents
{
    ...
    {
        [Test]
        public void OrdinaryActivityAtMeetingLocation_RestoresWithoutAMeetingOwner()
        {
            ...
            Assert.That(world.SubmitActivity(new NpcActivityRequest("ren", actor.WorldRunId, actor.Revision,
                "ren.rest", NpcActivity.Resting, 900)).IsSuccess, Is.True);
            ...
            var restoredWorld = world.PrepareRestore(RoundTrip(world.CaptureSnapshot()), Time(750, 2)).Value;

            var result = board.PrepareRestore(RoundTrip(board.CaptureSnapshot()), restoredWorld);

            Assert.That(result.IsSuccess, Is.True, result.ErrorCode);
            Assert.That(result.Value.GetCurrent("ren"), Is.Null);
            Assert.That(restoredWorld.GetState("ren").ActiveActivity.ActivityId, Is.EqualTo(world.GetState("ren").ActiveActivity.ActivityId));
        }
