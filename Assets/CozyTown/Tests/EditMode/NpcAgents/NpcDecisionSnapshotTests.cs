...
namespace CozyTown.Tests.EditMode.NpcAgents
{
    public sealed class NpcDecisionSnapshotTests
    {
        ...
        [TestCase(0.01, 12)]
        [TestCase(30, 0.01)]
        public void FractionalRealTime_CaptureRemainsValidAtTheConfiguredCooldownAndDecisionLimits(double cooldown, double decisionTimeout)
        {
            var world = World();
            var client = new ControlledClient();
            using var scheduler = new NpcDecisionScheduler(world, new[] { Profile() }, client,
                new NpcDecisionSettings(residentCooldownSeconds: cooldown, decisionTimeoutSeconds: decisionTimeout));
            scheduler.Tick(0.2);
            var saved = RoundTrip(scheduler.CaptureSnapshot(0.2));
            var restoredWorld = world.PrepareRestore(world.CaptureSnapshot(),
                new WorldTimeProgress(new GameClockSnapshot(1, 720), 0, false, 2)).Value;

            Assert.DoesNotThrow(() => scheduler.ValidateSnapshot(saved, restoredWorld, null));
            Assert.That(saved.Residents[0].RemainingCooldownSeconds, Is.LessThanOrEqualTo(cooldown));
            Assert.That(saved.Residents[0].Current.RemainingDecisionSeconds, Is.LessThanOrEqualTo(decisionTimeout));
        }

        [Test]
        public void LegacyReset_InitializesMissingBehaviorCooldownWithoutRefundingTheRollingBudget()
        {
            var client = new ControlledClient();
            using var scheduler = new NpcDecisionScheduler(World(), new[] { Profile() }, new UndeclaredClient(client),
                new NpcDecisionSettings(maxRequestsPerMinute: 2));
            ...
            scheduler.ResetWorld(World(), null, 2);
            scheduler.Tick(2);
        ...
        [Test]
        public void CurrentSocialDecision_WithAnUnconsumedCopyOfItsOpportunity_IsRejectedBeforePublication()
        {
            var world = WorldFor(new[] { "mina", "ren" });
            var plan = new NpcMeetingPlan("chat", "mina", "ren", "square", "mina.rest", "ren.rest", 720, 720, 780);
            var board = new NpcMeetingBoard(world, new[] { plan });
            var client = new ControlledClient();
            using var scheduler = new NpcDecisionScheduler(world, new[] { Profile() }, client, meetings: board);
            scheduler.Tick(0);
            var decisions = RoundTrip(scheduler.CaptureSnapshot(1));
            var savedBoard = board.CaptureSnapshot();
            var duplicatedOpportunity = new NpcMeetingBoardSnapshot(savedBoard.Plans, savedBoard.Meetings,
                savedBoard.LatestResults, savedBoard.LatestByResident, savedBoard.OfferedDays,
                new[] { new NpcMeetingOpportunitySnapshot("mina", "chat") }, savedBoard.Memories);
            var restoredWorld = world.PrepareRestore(world.CaptureSnapshot(),
                new WorldTimeProgress(new GameClockSnapshot(1, 720), 0, false, 2)).Value;
            var preparedBoard = board.PrepareRestore(duplicatedOpportunity, restoredWorld);
            Assert.That(preparedBoard.IsSuccess, Is.True, preparedBoard.ErrorCode);

            Assert.Throws<ArgumentException>(() => scheduler.ValidateSnapshot(decisions, restoredWorld, preparedBoard.Value));

            Assert.That(client.Tokens[0].IsCancellationRequested, Is.False);
            Assert.That(board.GetContext("mina"), Is.Null);
        }

        [Test]
        public void ExpiredPhysicalRequest_IsNotResumedByCapturingBeforeTheNextSchedulerTick()
        {
            var world = World();
            var client = new ControlledClient();
            using var scheduler = new NpcDecisionScheduler(world, new[] { Profile() }, client);
            scheduler.Tick(0);
            var saved = RoundTrip(scheduler.CaptureSnapshot(9));
            Assert.That(client.Tokens[0].IsCancellationRequested, Is.False, "Capturing an expired request must remain side-effect free.");
            var restoredWorld = world.PrepareRestore(world.CaptureSnapshot(),
                new WorldTimeProgress(new GameClockSnapshot(1, 720), 0, false, 2)).Value;
            client.Complete(0, new NpcDecisionReply(NpcDecisionKind.Wait));
