...
namespace CozyTown.Tests.EditMode.NpcAgents
{
    public sealed class NpcDecisionReplacementTests
    {
        ...
        [Test]
        public void ReadinessCallbackDisposingSource_PreventsReturningALiveReplacement()
        {
            var world = RestWorld("mina");
            using var firstClient = new ControlledClient();
            using var replacementClient = new ControlledClient();
            NpcDecisionScheduler scheduler = null;
            NpcDecisionScheduler replacement = null;
            using (scheduler = new NpcDecisionScheduler(world, Profiles("mina"), firstClient,
                canDecide: () => { scheduler.Dispose(); return true; }))
            {
                try
                {
                    Assert.Throws<ObjectDisposedException>(() =>
                        replacement = scheduler.CreateReplacement(Profiles("mina"), replacementClient));

                    Assert.That(replacement, Is.Null);
                    Assert.Throws<ObjectDisposedException>(() => scheduler.Tick(0));
                    Assert.That(firstClient.Requests, Is.Empty);
                    Assert.That(replacementClient.Requests, Is.Empty);
                }
