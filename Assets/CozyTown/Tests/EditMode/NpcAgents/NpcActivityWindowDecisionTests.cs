...
namespace CozyTown.Tests.EditMode.NpcAgents
{
    ...
    {
        [TestCase(1, 735, 0, 735, 45, 780)]
        [TestCase(1, 779, 0.5, 779.5, 0.5, 780)]
        [TestCase(2, 735, 0, 2175, 45, 2220)]
        public void OrdinaryRequest_DisclosesTheCurrentFractionalAndAbsoluteWindow(int day, int minute,
            double fraction, double opportunityTime, double maximum, double deadline)
        {
            var world = World(minute, fraction, "mina");
            if (day != 1) world.Observe(Time(minute, fraction, day));
            ...
            Assert.That(client.Requests[0].GameTotalMinutes, Is.EqualTo(opportunityTime));
            Assert.That(client.Requests[0].ActivityDeadlineTotalMinutes, Is.EqualTo(deadline));
            Assert.That(client.Requests[0].MaxActivityDurationGameMinutes, Is.EqualTo(maximum));
            Assert.That(world.GetState("mina").ActiveActivity, Is.Null);
        }

        [Test]
        public void OvernightRestWindow_EndsAtTheNextDaysWorkStart()
        {
            var schedule = new NpcDailySchedule("mina", "home", "outside", "entry", "work", "rest", "work",
                1200, 1260, 1380, 30, 120, 180);
            var world = new NpcAgentWorld(new[] { schedule });
            world.Observe(Time(1439, 0.5));
            using var client = new ControlledClient();
            using var scheduler = new NpcDecisionScheduler(world, new[] { Profile("mina") }, client);
            scheduler.Tick(0);
            Assert.That(client.Requests[0].ActivityDeadlineTotalMinutes, Is.EqualTo(1470));
            Assert.That(client.Requests[0].MaxActivityDurationGameMinutes, Is.EqualTo(30.5));
            client.Complete(0, new NpcDecisionReply(NpcDecisionKind.Visit, "work", NpcActivity.Working, 30.5));
            scheduler.Tick(1);
            world.Observe(Time(29, 0, 2));
            Assert.That(world.GetState("mina").ActiveActivity, Is.Not.Null);
            world.Observe(Time(30, 0, 2));
            Assert.That(world.GetState("mina").ActiveActivity, Is.Null);
        ...
        public void DirectClient_NonfiniteDurationCannotEnterTheWorldAndCanBeCorrected(double duration)
        {
            var world = World(735, 0, "mina");
            using var client = new ControlledClient();
            using var scheduler = new NpcDecisionScheduler(world, new[] { Profile("mina") }, client);
            scheduler.Tick(0);
            client.Complete(0, Visit("mina", duration));

            scheduler.Tick(1);

            Assert.That(world.GetState("mina").ActiveActivity, Is.Null);
            Assert.That(client.Requests.Count, Is.EqualTo(2));
            Assert.That(client.Requests[1].CandidateErrorCode, Is.EqualTo("candidate.duration_invalid"));
            client.Complete(1, Visit("mina", 10));
            scheduler.Tick(2);
            Assert.That(world.GetState("mina").ActiveActivity.ExpiresAtTotalMinutes, Is.EqualTo(745));
        ...
        [Test]
        public void Meeting_CrossesThePartnersScheduleAndReleasesIntoTheRemainingOrdinaryWindow()
        {
            var world = new NpcAgentWorld(new[]
            {
                new NpcDailySchedule("ren", "ren.home", "ren.outside", "ren.entry", "ren.work", "ren.rest",
                    "ren.afternoon", 360, 480, 720, 810, 1020, 1080),
                new NpcDailySchedule("sora", "sora.home", "sora.outside", "sora.entry", "sora.work", "sora.rest",
                    "sora.afternoon", 360, 480, 720, 780, 1020, 1080)
            });
            world.Observe(Time(720));
            var board = new NpcMeetingBoard(world, new[]
            {
                new NpcMeetingPlan("lunch", "ren", "sora", "square", "ren.rest", "sora.rest", 720, 750, 780)
            });
            var invitation = board.Invite(world.GetState("ren"), "lunch");
            Assert.That(invitation.IsSuccess, Is.True);
            Assert.That(board.Respond(world.GetState("sora"), invitation.Value.Id, true).IsSuccess, Is.True);
            world.TakeEvents("ren");
            world.TakeEvents("sora");

            world.Observe(Time(790));
            board.Observe();

            Assert.That(board.GetCurrent("ren").State, Is.EqualTo(NpcMeetingState.Travelling));
            Assert.That(world.GetState("sora").ActiveActivity.ExpiresAtTotalMinutes, Is.EqualTo(900));
            Assert.That(world.GetState("sora").Target.ExpectedActivity, Is.EqualTo(NpcActivity.Resting));
            using var client = new ControlledClient();
            using var scheduler = new NpcDecisionScheduler(world, new[] { Profile("ren"), Profile("sora") }, client,
                meetings: board);
            board.CancelAll();
            ...
            Assert.That(client.Requests.Count, Is.EqualTo(1));
            Assert.That(client.Requests[0].NpcId, Is.EqualTo("ren"));
            Assert.That(client.Requests[0].Social, Is.Null);
            Assert.That(client.Requests[0].GameTotalMinutes, Is.EqualTo(790));
            Assert.That(client.Requests[0].ActivityDeadlineTotalMinutes, Is.EqualTo(810));
            Assert.That(client.Requests[0].MaxActivityDurationGameMinutes, Is.EqualTo(20));
            client.Complete(0, Visit("ren", 20));
            scheduler.Tick(1);
            Assert.That(world.GetState("ren").ActiveActivity.ExpiresAtTotalMinutes, Is.EqualTo(810));
        ...
        private static WorldTimeProgress Time(int minute, double fraction = 0, int day = 1)
            => new WorldTimeProgress(new GameClockSnapshot(day, minute), fraction, false, 1);
