...
namespace CozyTown.Tests.UnityEditMode
{
    ...
    {
        [Test]
        public void SnapshotConfiguration_ExportsOnlyDeclaredModelFieldsInCanonicalOrder()
        {
            const string fields = "\"model\":\"deepseek-v4-flash\",\"promptVersion\":\"fixture-prompt-v1\","
                + "\"protocolVersion\":4,\"maxTokens\":512,\"thinking\":\"disabled\","
                + "\"responseFormat\":\"json_object\",\"stream\":false";
            var client = new ProxyNpcDecisionClient("https://agent.invalid/decide", snapshotConfiguration:
                "{" + fields + ",\"apiKey\":\"test-secret\",\"endpoint\":\"private.invalid\"}");
            var same = new ProxyNpcDecisionClient("https://agent.invalid/other", snapshotConfiguration:
                "{\"stream\":false," + fields.Replace(",\"stream\":false", "") + "}");

            Assert.That(client.SnapshotConfiguration, Does.Not.Contain("test-secret"));
            Assert.That(client.SnapshotConfiguration, Does.Not.Contain("private.invalid"));
            Assert.That(client.SnapshotConfiguration, Does.Contain("deepseek-v4-flash"));
            Assert.That(client.SnapshotConfiguration, Is.EqualTo(same.SnapshotConfiguration));
        }
