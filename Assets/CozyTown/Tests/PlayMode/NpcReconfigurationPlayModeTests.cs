...
namespace CozyTown.Tests.PlayMode
{
    public sealed class NpcReconfigurationPlayModeTests
    {
        ...
        public void UnfinishedRequest_RejectsReplacementUntilTheTaskEnds(bool timedOut)
        {
            ...
            first.Replies[0].SetResult(new NpcDecisionReply(NpcDecisionKind.Visit, "work", NpcActivity.Resting, 10));
            _controller.TickDecisions(timedOut ? 9 : 1);
            Assert.That(_controller.GetDecisionOutcome("npc.mina").ActivityAccepted, Is.EqualTo(!timedOut));
            _controller.ConfigureDecisions(replacement, Profiles());
        ...
        [Test]
        public void ProfileEnumeration_CannotDispatchOrReplaceTheBinding()
        {
            var first = Client();
            var replacement = Client();
            _controller.ConfigureDecisions(first, Profiles());
            var run = _controller.GetAgentState("npc.mina").WorldRunId;
            var other = CozyTownCompositionRoot.CreateDefault();
            _controller.ConfigureDecisions(replacement, EnumerateProfiles(() =>
            {
                _controller.TickDecisions(0);
                Assert.That(first.Requests, Is.Empty);
                Assert.Throws<InvalidOperationException>(() => _controller.Bind(other.WorldTimeFlow));
                Assert.Throws<InvalidOperationException>(() => _controller.ConfigureDecisions(first, Profiles()));
            }));

            Assert.That(_controller.GetAgentState("npc.mina").WorldRunId, Is.EqualTo(run));
            _controller.TickDecisions(0);
            Assert.That(replacement.Requests.Count, Is.EqualTo(1));
        }

        [Test]
        public void DispatchCallback_CannotReplaceOrDispatchAgainBeforeItsTaskIsRegistered()
        {
            var first = Client();
            var replacement = Client();
            ...
            first.OnRequest = () =>
            {
                if (entered) return;
                entered = true;
                try { _controller.ConfigureDecisions(replacement, Profiles()); }
            ...
            Assert.That(first.Requests.Count, Is.EqualTo(1));
            Assert.That(replacement.Requests, Is.Empty);
            Assert.That(_controller.ActiveDecisionRequests, Is.EqualTo(1));
            first.Replies[0].SetResult(new NpcDecisionReply(NpcDecisionKind.Visit, "work", NpcActivity.Resting, 10));
            _controller.TickDecisions(1);
        ...
        [Test]
        public void EnumerationThatFailsWorldRecovery_KeepsTheOldSchedulerAvailableForRecovery()
        {
            var first = Client();
            var replacement = Client();
            _controller.ConfigureDecisions(first, Profiles());
            Action<WorldTimeProgress> broken = _ => throw new InvalidOperationException("injected rebuild failure");
            _services.WorldTimeFlow.Changed += broken;
            try
            {
                Assert.Throws<InvalidOperationException>(() => _controller.ConfigureDecisions(replacement,
                    EnumerateProfiles(() => Assert.That(_services.GameSave.Load().ErrorCode, Is.EqualTo("save.loaded_rebuild_required")))));
                Assert.That(_controller.IsWorldReady, Is.False);
                _controller.TickDecisions(0);
                Assert.That(first.Requests, Is.Empty);
            }
            finally { _services.WorldTimeFlow.Changed -= broken; }
            Assert.That(_services.GameSave.Load().IsSuccess, Is.True);
            _controller.TickDecisions(1);
            Assert.That(first.Requests.Count, Is.EqualTo(1));
            Assert.That(replacement.Requests, Is.Empty);
        ...
        [Test]
        public void CompletedTaskReplacement_DoesNotInvokeItsCancellationCallbackOrApplyItsReply()
        {
            var first = Client();
            _controller.ConfigureDecisions(first, Profiles());
            _controller.TickDecisions(0);
            ...
            var run = _controller.GetAgentState("npc.mina").WorldRunId;
            using (first.Tokens[0].Register(() => { cancelled = true; _services.GameSave.Load(); }))
            {
                first.Replies[0].SetResult(new NpcDecisionReply(NpcDecisionKind.Visit, "work", NpcActivity.Resting, 10));
        ...
        private static IEnumerable<NpcDefinition> EnumerateProfiles(Action before)
        {
            before();
            foreach (var profile in Profiles()) yield return profile;
        }
