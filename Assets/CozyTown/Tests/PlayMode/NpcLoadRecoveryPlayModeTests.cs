...
namespace CozyTown.Tests.PlayMode
{
    public sealed class NpcLoadRecoveryPlayModeTests
    {
        ...
        public void PostCommitFailure_ReportsCommittedSaveAndFreezesDecisionsUntilReload(string fault)
        {
            ...
            var travelling = _controller.GetAgentState(Sora);
            Assert.That(_controller.SubmitActivity(new NpcActivityRequest(Sora, travelling.WorldRunId, travelling.Revision,
                Sora + ".outside", NpcActivity.Resting, 800)).IsSuccess, Is.True);
            _services.DaytimeClock.AdvanceElapsed(0.75);
            Assert.That(_sora.Position, Is.EqualTo(new Vector2(40, 18.5f)));
            if (fault == "after") _services.WorldTimeFlow.Changed += broken;
            var animator = fault == "body" ? _ren.gameObject.AddComponent<CozyTownNpcSpriteAnimator>() : null;
            ...
            Assert.That(_controller.IsWorldReady, Is.False);
            Assert.That(_services.WorldTimeFlow.NotificationFailures.Count, Is.EqualTo(1));
            Assert.That(_sora.Position, Is.EqualTo(new Vector2(40, fault == "body" ? 18.5f : 20)));
            ...
            var current = _controller.GetAgentState(Sora);
            Assert.That(_controller.SubmitActivity(new NpcActivityRequest(Sora, current.WorldRunId, current.Revision,
                Sora + ".outside", NpcActivity.Resting, 800)).ErrorCode, Is.EqualTo("agent.world_not_ready"));
            Assert.That(_controller.CancelActivity(Sora, current.WorldRunId, current.Revision).ErrorCode, Is.EqualTo("agent.world_not_ready"));
            Assert.Throws<InvalidOperationException>(() => _controller.ConfigureDecisions(_client,
                new[] { new NpcDefinition(Sora, "Sora", "Cook", "Hi") }));
        ...
        [TestCase(true)]
        [TestCase(false)]
        public void ClientReentrantSynchronizationFailure_StopsFurtherDispatchInTheCurrentTick(bool load)
        {
            Assert.That(_services.GameSave.Save().IsSuccess, Is.True);
            _controller.ConfigureDecisions(_client, new[] {
                new NpcDefinition(Sora, "Sora", "Cook", "Hi"), new NpcDefinition(Ren, "Ren", "Fisher", "Hi") },
                new NpcDecisionSettings(maxRequestsPerMinute: 3, maxConcurrentRequests: 2));
            _services.WorldTimeFlow.Changed += progress => throw new InvalidOperationException("injected");
            _client.OnRequest = () => { if (load) _services.GameSave.Load(); else _services.DaytimeClock.AdvanceElapsed(0.25); };

            _controller.TickDecisions(0);

            Assert.That(_services.WorldTimeFlow.State, Is.EqualTo(WorldTimeFlowState.RecoveryRequired));
            Assert.That(_client.Requests.Count, Is.EqualTo(1));
            Assert.That(_controller.ActiveDecisionRequests, Is.EqualTo(1));
            Assert.That(_client.Tokens[0].IsCancellationRequested, Is.True);
        ...
        [TestCase(false)]
        [TestCase(true)]
        public void RestoreFailureWithoutConfirmedRollback_FreezesTheBoundNpcWorld(bool throws)
        {
            Assert.That(_services.GameSave.Save().IsSuccess, Is.True);
            int savedSeed = _services.WorldSeed.Value;
            StartInvitation();
            _services.WorldSeed.Restore(777);
            var oldRun = _controller.GetAgentState(Sora).WorldRunId;
            var seed = new FailingSeed(_services.WorldSeed, throws);
            var save = new GameSaveCoordinator(seed, _services.Time, _services.EconomyState,
                _services.Farm, _services.Livestock, _services.SaveStorage);
            var clock = new DaytimeClockCoordinator(_services.WorldTime, save, (WorldTimeFlow)_services.WorldTimeFlow);
            OperationResult result = default;

            Assert.DoesNotThrow(() => result = clock.Load());

            Assert.That(result.ErrorCode, Is.EqualTo(throws ? "save.restore_exception" : "save.rollback_world_seed_failed"));
            Assert.That(_services.WorldSeed.Value, Is.EqualTo(savedSeed), "The failed operation already changed a module.");
            Assert.That(_services.WorldTimeFlow.State, Is.EqualTo(WorldTimeFlowState.RecoveryRequired));
            Assert.That(_controller.GetAgentState(Sora).WorldRunId, Is.EqualTo(oldRun), "No complete snapshot was published.");
            _controller.TickDecisions(1);
            Assert.That(_client.Tokens[1].IsCancellationRequested, Is.True);
            Assert.That(_controller.ActiveDecisionRequests, Is.EqualTo(1));
            Assert.That(_services.WorldTime.AdvanceMinutes(1).ErrorCode, Is.EqualTo("world_time.not_ready"));
            Assert.That(_services.GameSave.Save().ErrorCode, Is.EqualTo("world_time.not_ready"));
            Assert.That(_services.GameSave.Load().IsSuccess, Is.True);
            Assert.That(_controller.GetAgentState(Sora).WorldRunId, Is.Not.EqualTo(oldRun));
        }

        [Test]
        public void ReplyCompletedDuringRecovery_CannotDeliverAndDoesNotResetBudget()
        {
            Assert.That(_services.GameSave.Save().IsSuccess, Is.True);
            StartInvitation();
            ...
            Action<WorldTimeProgress> broken = progress => throw new InvalidOperationException("injected");
            _services.WorldTimeFlow.Changed += broken;
            Assert.That(_services.GameSave.Load().ErrorCode, Is.EqualTo("save.loaded_rebuild_required"));
            _controller.TickDecisions(3);
            Assert.That(_controller.ActiveDecisionRequests, Is.EqualTo(1));
            ...
            Assert.That(_client.Requests[3].Self.WorldRunId, Is.EqualTo(_controller.GetAgentState(Sora).WorldRunId));
        }

        [Test]
        public void RejectedRestore_CallbackCannotMutateOrConsumeOriginalDecision()
        {
            StartInvitation();
            var oldRun = _controller.GetAgentState(Sora).WorldRunId;
            var outcomes = new List<string>();
            var save = new RejectedSave(() =>
            {
                _controller.TickDecisions(1);
                outcomes.Add(_services.GameSave.Save().ErrorCode);
                outcomes.Add(_services.GameSave.Load().ErrorCode);
            ...
            Assert.That(clock.Load().ErrorCode, Is.EqualTo("save.payload_invalid"));

            Assert.That(outcomes, Is.EqualTo(new[] { "world_time.not_ready", "world_time.not_ready", "world_time.not_ready" }));
            Assert.That(_controller.IsWorldReady, Is.True);
            Assert.That(_client.Tokens[1].IsCancellationRequested, Is.False);
            Assert.That(_controller.GetAgentState(Sora).WorldRunId, Is.EqualTo(oldRun));
            _client.Replies[1].SetResult(new NpcDecisionReply(NpcDecisionKind.AcceptInvitation,
                meetingId: _controller.GetMeeting(Ren).Id));
            _controller.TickDecisions(2);
        ...
        private sealed class RejectedSave : IGameSaveCoordinator
        {
            private readonly Action _onLoad;
            public RejectedSave(Action onLoad) { _onLoad = onLoad; }
            public bool HasSave => true;
            public OperationResult Save() => OperationResult.Success();
            public OperationResult Load() { _onLoad(); return OperationResult.Failure("save.payload_invalid"); }
        }

        private sealed class FailingSeed : IWorldSeedState
        {
            private readonly IWorldSeedState _inner;
            private readonly bool _throws;
            private int _calls;
            public FailingSeed(IWorldSeedState inner, bool throws) { _inner = inner; _throws = throws; }
            public int Value => _inner.Value;
            public OperationResult Restore(int value)
            {
                if (++_calls == 1) _inner.Restore(value);
                if (_throws) throw new InvalidOperationException("injected restore failure");
