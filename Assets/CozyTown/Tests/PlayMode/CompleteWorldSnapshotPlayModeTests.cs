...
namespace CozyTown.Tests.PlayMode
{
    public sealed partial class CompleteWorldSnapshotPlayModeTests
    {
        ...
        private void CreateWorld(bool sharedMeetingLocations = false)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                float offset = i * (sharedMeetingLocations ? 2f : 50f);
                string npc = npcIds[i];
                string home = "home." + i;
                ...
                locations.Add(new TownLocation(outside, new Vector2(offset, 0)));
                locations.Add(new TownLocation(entry, new Vector2(offset, 1)));
                locations.Add(new TownLocation(work, new Vector2(offset + 20f, 0)));
                locations.Add(new TownLocation(rest, new Vector2(offset + 20f, 10)));
            ...
            _controller.ConfigureSnapshots(_services.WorldSnapshots, _player, _gate, realSeconds: () => _realSeconds);
        }
    }
}
