...
项目提供了一套可运行、可存档、可自动化验证的小镇生活循环：玩家可以种植、养鸡、钓鱼、烹饪、交易，并与场景中的 NPC 对话。四人实验入口还支持内置事件触发的自主活动、邀约、会面和资源交付；模型提出候选，确定性游戏代码检查权限、时效和资产后执行。

第一版 AI Agent 实验平台候选位于 `codex/agent-platform-candidate`，交付 PR 保持 Draft、未合并。使用 Unity 的 **CozyTown > Agent Experiment** 打开四人实验；环境、外部凭据模板和操作步骤见[实验指南](docs/AGENT_EXPERIMENT_GUIDE.md)，来源、依赖、测试及已知限制见[候选验收记录](docs/verification/agent-platform-candidate-2026-09-20.md)。普通场景与显式四人实验使用不同配置。
...
以上描述玩家对话链路。四人实验通过局部观察和渐进式披露构建决策上下文，共享有界调度器，候选只能使用当前允许的行动。实验提供 F（自由台词）／S（结构化事实表达）显式选择、完整逻辑快照、角色观察、导出及严格 Replay。真实矩阵的 16 个世界中 15 个完成并通过回放，1 个期限未完成；这不代表所有会面或台词均正确，具体结果见[集成验证](docs/verification/four-agent-integration-2026-09-20.md)。
...
2026-09-09 昼夜天色已接入六个平滑渐变的时间点，早上至中午共用白天关键点。道路与门前的 18 盏路灯在 18:00–19:00 渐亮、05:00–06:00 渐灭，并跟随暂停、睡眠和读档。全量 EditMode `491/491`、图形 PlayMode `149/149` 通过；[天色配置与正式场景截图](docs/DAY_NIGHT_LIGHTING.md)可用于复查颜色和灯位。
...
- [`docs/DAY_NIGHT_LIGHTING.md`](docs/DAY_NIGHT_LIGHTING.md)：天色时间点、路灯配置、正式场景截图与验证记录
- [`docs/TOWN_LIFE_PLAN.md`](docs/TOWN_LIFE_PLAN.md)：T1 扩镇与日常验收契约；四人作息已实现，人工场景验收待确认
- [`docs/TOWN_LIFE_IMPLEMENTATION.md`](docs/TOWN_LIFE_IMPLEMENTATION.md)：居民模块、个人时间表、TDD 与全量回归证据
