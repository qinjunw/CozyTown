...
- 当前状态：两个 EditMode 测试程序集包含 183 个用例，PlayMode 测试程序集包含 35 个用例；2026-08-31 完成 Scene-01e 后，全量批处理结果为 EditMode `183/183`、PlayMode `35/35` 通过，均为 0 failed、0 skipped。
- 当前边界：M4 已自动化 schema v1 JSON 单槽存档、五模块保存恢复与回滚、AI 代理编解码与故障回退、状态越权隔离和 Presenter 生命周期。A1 已完成 Production 资源门禁及 Scene-01a 至 Scene-01e；自动化已停在人工场景验收前。真实 AI 驱动 NPC、30 条评测、延迟与成本统计、Windows 构建和录屏在 Scene-01 验收后进入 M5。
...
当前全量回归包含 183 个 EditMode 和 35 个 PlayMode 用例，覆盖组合根、时间溢出、背包容量、交易与生产模块回滚、只读玩法投影、连续跨日、默认内容可达性、配置集合隔离、schema v1 文件往返与原文件保护、五模块存档回滚、AI 故障回退和状态越权隔离、A1 Production 资源契约，以及 Unity 像素相机、Tilemap、Production 场景引用、玩家动画、农田和母鸡状态、真实 Physics2D 边界、HUD、七种交互语义、10 个物理交互实体、模态输入生命周期、晚注册、4 名独立世界 NPC、池塘四向交互优先级、存档入口、Production UGUI 按钮接线、三档静态参考坐标边界和正式场景完整闭环。当前没有第二个 schema 版本，因此不存在旧版本迁移夹具；引入 v2 前必须先补充迁移测试。
