...
- 当前状态：两个 EditMode 测试程序集包含 224 个用例，PlayMode 测试程序集包含 35 个用例；2026-09-02 角色—商店原子出售切片后的全量 EditMode 结果为 `224/224`，2026-09-01 最近一次全量 PlayMode 结果为 `35/35`，均为 0 failed、0 skipped。
...
当前全量回归包含 224 个 EditMode 和 35 个 PlayMode 用例，覆盖组合根、时间溢出、背包容量、稳定角色/商店经济状态存储、显式角色与商店身份的守恒购买和出售、交易与生产模块回滚、只读玩法投影、连续跨日、默认内容可达性、配置集合隔离、schema v1 文件往返与原文件保护、五模块存档回滚、AI 故障回退和状态越权隔离、A1 Production 资源契约，以及 Unity 像素相机、Tilemap、Production 场景引用、玩家动画、角色 `24×32` 图集及其包络和左右镜像轮廓、建筑屋顶前景、农田和母鸡状态、真实 Physics2D 边界、HUD、七种交互语义、10 个物理交互实体、模态输入生命周期、晚注册、4 名独立世界 NPC、池塘四向交互优先级、存档入口、Production UGUI 按钮接线、三档静态参考坐标边界和正式场景完整闭环。当前没有第二个 schema 版本，因此不存在旧版本迁移夹具；引入 v2 前必须先补充迁移测试。
