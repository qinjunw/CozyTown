...
- 当前状态：两个 EditMode 测试程序集包含 158 个用例，PlayMode 测试程序集包含 26 个用例；2026-08-29 的 A1 资源批处理结果分别为 158 passed 与 26 passed，均为 0 failed、0 skipped。
- 当前边界：M4 已自动化 schema v1 JSON 单槽存档、五模块保存恢复与回滚、AI 代理编解码与故障回退、状态越权隔离、对话/存档 Presenter 生命周期，以及占位场景装配。A1 已完成 Production 资源门禁，Scene-01 场景接线待完成。真实 AI 驱动 NPC、30 条评测、延迟与成本统计、Windows 构建和录屏在 Scene-01 验收后进入 M5。
...
当前 184 个用例覆盖组合根、时间溢出、背包容量、交易与生产模块回滚、只读玩法投影、连续跨日、默认内容可达性、配置集合隔离、schema v1 文件往返与原文件保护、五模块存档回滚、AI 故障回退和状态越权隔离、A1 Production 资源契约，以及 Unity 移动、碰撞、HUD、七点交互、模态输入生命周期、晚注册、4 名 NPC、存档入口和正式场景完整闭环。当前没有第二个 schema 版本，因此不存在旧版本迁移夹具；引入 v2 前必须先补充迁移测试。
