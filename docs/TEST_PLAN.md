...
- 当前状态：两个 EditMode 测试程序集包含 190 个用例，PlayMode 测试程序集包含 35 个用例；2026-09-01 完成 Scene-01g 后，全量批处理结果为 EditMode `190/190`、PlayMode `35/35` 通过，均为 0 failed、0 skipped。
- 当前边界：M4 已自动化 schema v1 JSON 单槽存档、五模块保存恢复与回滚、AI 代理编解码与故障回退、状态越权隔离和 Presenter 生命周期。A1 已完成 Production 资源门禁及 Scene-01a 至 Scene-01g；自动化已停在人工场景验收前。真实 AI 驱动 NPC、30 条评测、延迟与成本统计、Windows 构建和录屏在 Scene-01 验收后进入 M5。
