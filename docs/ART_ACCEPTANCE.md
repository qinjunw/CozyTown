...
- 自动回归：EditMode `166/166`、PlayMode `26/26` 通过；日志未发现 C# 编译错误、测试失败或未处理异常。
- 布局与幂等：10 个正式面板在 `320×180`、`640×360`、`1280×720` 的自动边界检查通过；场景升级器连续运行前后的 `CozyTown_Dev.unity` SHA-256 相同。
