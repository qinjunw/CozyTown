...
Scene-01 按六个垂直切片验收，任一切片未通过时不得把正式场景标记为完成：
...
| A1 Production 清单测试 | 初始 Red 精确报告 11 个缺失文件；现行资源目标测试 7/7 通过 |
| A1 可切片资源 | 13 个 PNG、103 个 Sprite、13 个 4× 预览已生成；包含独立齿轮和四人世界图集 |
| 正式场景替换 | Scene-01a/01b/01c/01d/01e/01f 自动化已完成；最终人工场景验收待执行 |
