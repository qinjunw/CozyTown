...
生产 Sprite 的画布必须与声明的行列和单帧尺寸完全一致。Scene-01g 世界角色帧为 `24×32 px`，脚底中心 Pivot 保持一致；物品图标为 `16×16 px`。所有 Sprite 名称使用稳定 ID 的资源形式，例如 `npc_shopkeeper_mina_idle_down`、`item_seed_potato` 和 `crop_tomato_stage_04`。
...
Scene-01 按八个垂直切片验收，任一切片未通过时不得把正式场景标记为完成：
...
| A1 可切片资源 | 13 个 PNG、106 个 Sprite、13 个 4× 预览已生成；包含独立齿轮、屋顶前景和四人世界图集 |
| 正式场景替换 | Scene-01a/01b/01c/01d/01e/01f/01g/01h 自动化已完成；最终人工场景验收待执行 |
