...
namespace CozyTown.Tests.EditMode.Save
{
    public sealed class CompleteWorldSaveTests
    {
        ...
        [TestCase(false)]
        [TestCase(true)]
        public void RejectedWorldPreparation_KeepsTheRunningClockAndDoesNotCommit(bool unexpectedException)
        {
            var services = CozyTownCompositionRoot.CreateDefault();
            ...
            var adapter = new RejectingWorldAdapter(unexpectedException);
        ...
        private sealed class RejectingWorldAdapter : IWorldSnapshotAdapter
        {
            ...
            public RejectingWorldAdapter(bool unexpected) => _unexpected = unexpected;
            ...
            {
                Prepared = true;
                if (_unexpected) throw new NotSupportedException("Candidate preparation failed before any publication.");
