...
namespace CozyTown.Tests.PlayMode
{
    public sealed class BodySnapshotPlayModeTests
    {
        ...
        [Test]
        public void OrdinaryBlockedSnapshot_RejectsOffRoadPositionInsteadOfShowingAResidentThere()
        {
            var map = CreateMap();
            var invalid = new TownRouteSnapshot(new Position2DSnapshot(50f, 50f),
                new Position2DSnapshot(0f, -1f), "destination", Array.Empty<Position2DSnapshot>(),
                1, (int)TownRouteStatus.Blocked, true);

            Assert.Throws<ArgumentException>(() => TownRouteFollower2D.RestoreSnapshot(map, invalid,
                0.3f, Vector2.zero, _world.transform));
        }
    }
}
