...
Scene-01 分为世界静态表现、运行时状态表现、UI 皮肤、常驻 UI 信息架构和场景语义边界五个垂直切片。Scene-01a 至 Scene-01e 自动化均已完成；正式界面使用 UGUI 消费 Production 九宫格、按钮状态和图标，常驻区域收敛为左上 HUD、目标 `E` 气泡、五格快捷栏与无底框灰色齿轮。六个独立实体障碍位于交互点父链之外；四名 NPC 使用各自的稳定 ID、世界 Sprite 和 Presenter，共享同一个只显示当前 NPC 的对话 View。Scene-01 仍需人工确认实际画面的可读性、门口手感、岸线贴合、接缝、脚底稳定和整体构图。真实 AI 驱动 NPC、模型评测和联网诊断在人工验收后进入 M5。
