...
Scene-01 分为世界静态表现、运行时状态表现、UI 皮肤和常驻 UI 信息架构四个垂直切片。四个自动化切片均已完成；正式界面使用 UGUI 消费 Production 九宫格、按钮状态和图标，常驻区域收敛为左上 HUD、目标 `E` 气泡、五格快捷栏与右上齿轮。包裹和系统菜单共享既有模态输入门，退役的 `OnGUI` 与左下持续提示路径已删除。Scene-01 仍需人工确认实际画面的可读性、接缝、脚底稳定和整体构图。真实 AI 驱动 NPC、模型评测和联网诊断在人工验收后进入 M5。
