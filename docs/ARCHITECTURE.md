...
Scene-01 分为世界静态表现、运行时状态表现和 UI 皮肤三个垂直切片。调试 `OnGUI` 只可在对应 UI 切片完成前作为过渡界面；Scene-01 最终验收必须明确哪些 Production UI 资源已被正式界面消费。真实 AI 驱动 NPC、模型评测和联网诊断在 Scene-01 验收后进入 M5。
...
9. 正在进行：完成 A1 Scene-01 正式场景接线与验收，不改变既有玩法和 AI 权限边界。
