...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.23，2026-09-17 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 读档提交后关键同步失败进入恢复状态；隔离显示通知，暂停时间协调、保存及 NPC 行动，修复后通过有效重读档恢复 |
| 代码参照 | 基于 `ee5f4d7ca387b9130d8e6e3a016095d458043ac9`；恢复边界见 [ADR-0020](adr/0020-post-commit-world-recovery.md)，当前存档仍为 schema v3 |
| 证据方法 | 公开入口注入通知、组件及恢复模块故障，以受控请求检查旧回复、并发和预算；结果见[读档恢复验证](verification/npc-load-recovery-2026-09-17.md)。真实模型调用为 0；完整快照的 13 组验收仍属后继实现 |
