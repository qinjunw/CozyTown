...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.20，2026-09-17 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 盘点完整逻辑快照的状态与恢复边界，形成契约及 ADR 草案；精确位置、进行中互动和旧档兼容的三个恢复选择待用户答复 |
| 代码参照 | 本轮核查冻结于 `7d20bad55a854fb496a8cb295ae93648e66d6141`；当前存档仍为 schema v3；生产和测试代码未修改 |
| 证据方法 | 本轮只读核对世界、资产、Agent、身体、时钟及已有测试入口，新增运行测试和真实模型调用均为 0，见[快照契约草案](AGENT_SNAPSHOT_SPEC.md)。前次结果见[装配核验](audits/npc-session-composition-2026-09-15.md)及[续谈修复验证](verification/npc-conversation-turn-2026-09-14.md)，均为历史证据 |
...
2026-09-17，用户确认本轮用途为“四人自主运行、完整逻辑快照、可观测和重复实验”的 AI Agent 实验平台。[第一版范围与完成条件](wayfinder/agent-world/platform-milestone.md)规定本轮交付边界：沿用现有地图和已实现行动，补齐逻辑状态恢复、四人实验配置、角色观察与固定输入回放，并在同一候选提交上验收。新增职业生产闭环、需求和长期关系系统留待新的范围决策。精确位置、进行中会面续接、普通活动期限与表达模式判定仍由对应决策票据确定。
...
完整逻辑快照已纳入第一版目标。[契约草案](AGENT_SNAPSHOT_SPEC.md)列出字段分类、同一时刻捕获、逻辑待办续接、旧请求隔离及验收矩阵；[ADR-0019](adr/0019-complete-logical-snapshots-and-agent-continuation.md)仍为 Proposed。三个可见恢复选择待用户答复，当前 schema v3 尚未保存 Agent 活动、会面与经历。
