...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.8，2026-09-13 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 资源协议 v4 披露自方缺口、限制允许动作，并在邀约／接受执行时复核本人资产；12 轮真实场景验证资源行为，保留 1 轮恢复失败 |
| 代码参照 | 自方条件实现 `1e538b84c8a0e18de50ec2a08fd1bc4515811fb2`；历史研究、实施与实验以各阶段报告的独立版本为准 |
| 证据方法 | 公开行为边界回归、实际 Unity 场景和原始模型输出；本轮真实调用 42 次，初始化与资产检查 12/12，恢复 11/12，真实矩阵整体失败 |
