...
| 本轮交付范围 | 在局部观察上新增有限发言意图、事实引用校验、宿主整句渲染与两组表达实验；普通游戏仍默认自由表达 |
| 代码参照 | 表达实现冻结于 `c8bf837d1a30f358970566bd06d91ebec8645a87`；输入、标注与运行结果见[表达实验](verification/npc-expression-comparison-2026-09-14.md) |
| 证据方法 | Python 60、EditMode 801、PlayMode 190 项通过；48 个固定真实样本与 4 个新世界共 56 次 v4-flash 调用；固定宿主输出与实际场景发布分开统计 |
| 未验证内容 | 结构化版本错误修复后的可用性、移动中答复恢复后的真实交谈、长期可靠率、四人共同负载、长期角色差异及跨日持久化稳定性 |
