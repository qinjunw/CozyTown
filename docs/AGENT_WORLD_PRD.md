...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.9，2026-09-13 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 候选必填字段、JSON 类型和请求绑定检查；宿主最多一次纠正，共用原预算与期限，保留首次错误 |
| 代码参照 | 候选校验实现 `8f5be22f3c36ccb36118e8775a4e0bfd533b84f9`；历史研究、实施与实验以各阶段报告的独立版本为准 |
| 证据方法 | 公开行为边界回归、实际 Unity 场景及原始模型输出；12 轮共 47 次真实调用，6 次注入错误全部纠正；初始化、资产、恢复各 12/12，但 1 次交付后会面到期 |
