...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.19，2026-09-17 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 根据用户确认的实验平台用途，规定四人自主运行、完整逻辑快照、可观测和重复实验的阶段完成条件，并调整 GitHub 任务范围与依赖 |
| 代码参照 | 本轮核查冻结于 `aebdcce1b1d3c7d48bc6d1ff9e1a989a64634f9c`；当前存档仍为 schema v3；生产和测试代码未修改 |
| 证据方法 | 本轮核对源码、PRD 和 GitHub 票据，新增 Unity 测试运行和真实模型调用均为 0。存档现状见[生命周期研究](research/npc-agent/save-load-lifecycle-2026-09-16.md)；前次 26 项复测见[装配核验](audits/npc-session-composition-2026-09-15.md)，完整回归及四世界 18 次调用见[续谈修复验证](verification/npc-conversation-turn-2026-09-14.md)，均为历史结果 |
...
完整逻辑快照已纳入第一版目标。具体字段、进行中行动恢复和兼容规则由快照契约票据确定，随后实施；当前 schema v3 尚未保存 Agent 活动、会面与经历。
