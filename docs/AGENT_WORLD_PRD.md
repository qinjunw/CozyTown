...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.26，2026-09-20 |
| 状态 | 四人实验入口、完整逻辑快照、角色观察和严格回放已交付；跨日读档矩阵已完成评测，保留未完成世界及行为限制；候选验收见[冻结记录](verification/agent-platform-candidate-2026-09-20.md) |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 按[第一版范围](wayfinder/agent-world/platform-milestone.md)验收已有行动下的四人 AI Agent 实验平台；新增玩法和长期关系另立范围 |
| 代码参照 | 候选入口测试来源 `bc59db8de897261c3385286baf2e6cedf6b9677b`；代码与配置树等同集成实测版本，最终文档提交由候选票据 Resolution 登记；完整存档为 schema v4，PR 栈未合并 |
| 证据方法 | 完整快照[实现验证](verification/complete-agent-snapshots-2026-09-19.md)、[四人集成评测](verification/four-agent-integration-2026-09-20.md)和候选入口 12 项 PlayMode 检查分别绑定实际运行来源；16 个真实世界保留 15 个完成及 1 个期限未完成 |
| 未验证内容 | 长期可靠率、长期角色差异、全部六种人物配对与更大规模负载；台词事实正确性和表达质量不能由流程完成证明，第二天观察过期拒绝仍是已测限制 |
...
同日第二轮结果：默认休息窗口可以通过世界事件自动产生决策；调度器限制调用、并发、续调与有效期，按需披露本人已知地点，再将候选交给活动仲裁。Unity 帧更新已驱动实际人物，独立 HTTP 代理协议支持显式接入。共享工作区 EditMode 557/557、图形 PlayMode 172/172 通过；发布采用隔离检出复测，证据见[第二轮验证](verification/agent-decisions-2026-09-12.md)、[ADR-0016](adr/0016-bounded-autonomous-decisions.md) 和[代理协议](NPC_AGENT_PROTOCOL.md)。该阶段尚未实现代理服务端、社交承诺或资源协作。

第三轮结果：世界机会与会面阶段自动唤醒相应居民，双方接受后预约未来活动及地点，实际到场后开放有界交谈。取消、拒绝、到期、玩家打断和读档均由宿主处理；最近经历只保留在本人内存中。真实联调请求 `deepseek-v4-flash`，服务返回 `deepseek-flash`，6 次调用完成两人会面；对话出现时间称呼错误及未确认鱼获，不能据此认定模型已经可靠掌握世界事实。完整验证、用量和限制见[第三轮报告](verification/agent-meetings-2026-09-12.md)；决策见 [ADR-0017](adr/0017-resident-meeting-commitments.md)。

第四轮结果：Sora 缺鱼的内置条件触发资源会面，双方同意并实际到场后，宿主一次交换 1 条鲤鱼和 25 金币并记录交付结果。NPC 资产可保存、恢复和用于现有配方，旧时间线及重复交付被拦截。隔离检出 EditMode 614 项通过、图形 PlayMode 173 项通过且 2 项真实服务测试默认跳过；显式真实资源场景通过，6 次调用共 6,072 Token。未确认的香草和鱼群叙述仅保留为台词，不生成资产或生产结果。完整边界与证据见[第四轮报告](verification/agent-resources-2026-09-12.md)和 [ADR-0018](adr/0018-resident-resource-delivery.md)。

2026-09-13 自方条件实施：基于上下文实验采用本人缺口披露、阶段选项过滤及提交时资产复核，资源协议升为 v4。隔离 EditMode 638 项、PlayMode 175 项和 Python 26 项通过，4 项真实服务入口默认跳过。12 轮真实矩阵中资源行为符合本轮规则、资产检查全部通过，但一轮普通工作活动持续 720 分钟，在 16:40 尚未结束，恢复检查 11/12，真实入口退出 2；另有一个过期对话响应被丢弃。详见[实施验证](verification/npc-resource-preconditions-2026-09-13.md)。动作过滤不计为模型推理改善，普通活动期限与恢复验收留待独立决策。
...
2026-09-17，用户确认本轮用途为“四人自主运行、完整逻辑快照、可观测和重复实验”的 AI Agent 实验平台。[第一版范围与完成条件](wayfinder/agent-world/platform-milestone.md)规定本轮交付边界：沿用现有地图和已实现行动，补齐逻辑状态恢复、四人实验配置、角色观察与固定输入回放，并在同一候选提交上验收。新增职业生产闭环、需求和长期关系系统留待新的范围决策。用户已进一步采纳精确位置与路线恢复、进行中会面续接及旧档初始化规则；2026-09-19 确认普通自主活动限于当前日程窗口，超限允许一次有界纠正。表达模式判定继续由对应票据确定。

## 3. 功能与运行需求

P0 表示首个自主闭环的必要条件；P1 表示四人世界的扩展要求。第一版实验平台已纳入 FR-014、015、016 所需的观察、四人实验和完整逻辑快照；其余候选玩法不因本次范围确认自动获得实施状态。阶段定义见第 13 节。
...
| AW-FR-016 | P1／M5 | 完整逻辑快照覆盖已实现且影响后续行为的状态；逐项规定保存或重建方式、进行中行动的恢复语义和旧格式兼容，隔离旧响应及运行期限额。 | AC-07 |

上表验收缩写 `AC-xx` 均指 `AW-AC-xx`。双人实验已有延迟和用量记录，四人持续负载下的性能与体验验收阈值尚未验证；第 9 节数值只能作为实验配置，不能直接写成达标结论。
...
下表保留总体阶段；第一版剩余任务与停止条件以[实验平台计划](wayfinder/agent-world/platform-milestone.md)为准。每个任务记录覆盖的 `AW-FR`、`AW-AC`、采用的决策及验证结果。
...
| M5 四人实验平台 | 已有 M1–M4；依次明确快照契约、修复恢复边界、实现持久化与实验入口，再做集成评测 | 同一候选提交具备四人自主运行、完整逻辑快照、角色观察、固定输入回放及真实模型分布报告；干净检出可复验 |
...
完整逻辑快照已纳入第一版目标。[契约 1.0](AGENT_SNAPSHOT_SPEC.md)规定字段分类、同一时刻捕获、精确位置与互动续接、旧档初始化、逻辑待办续接、旧请求隔离及 13 组验收；[ADR-0019](adr/0019-complete-logical-snapshots-and-agent-continuation.md)已接受。目标契约尚待实现，当前 schema v3 尚未保存 Agent 活动、会面与经历。
...
### B-003：使用 v4-flash 和真实测试密钥推进自主会面

- 日期：2026-09-12。
- 用户原话：“就用v4-flash就好,你继续下一阶段,并且可以用我的真实key测试,注意工作规范和原则”。
- 讨论结果：请求模型固定为 `deepseek-v4-flash`，实际服务返回的型号另行记录；密钥由仓库外配置供本地代理读取。第三轮完成后续资源协作所需的会面与轮次机制。
- 实测反馈：游戏执行链完成；台词的时间及资源事实一致性仍需评测。改进方向包括显式时段披露、已确认资源事实和跨多次运行的语义检查，具体方案留在后续任务讨论。
- 关联：AW-AC-01、AW-FR-008、009、010；[自主会面](https://github.com/qinjunw/CozyTown/issues/53)、[四人跨日与读档恢复](https://github.com/qinjunw/CozyTown/issues/55)。

### B-004：继续接通角色资源与实际协作

- 日期：2026-09-12。
- 用户原话：“好的,开始下一阶段”。
- 讨论结果：按既定顺序推进资源协作，并沿用 v4-flash 与有界真实测试授权。首个切片使用固定鲤鱼价款和明确初始资产；这些是本轮实现选择，不代表全部经济玩法已定稿。
- 实测反馈：实际资产守恒、角色恢复日程；自动烹饪尚未加入模型协议，经历与进行中承诺的持久化及多次模型评测继续留在后继任务。
- 关联：AW-UC-02、AW-FR-011、AW-AC-08；[接通角色资源与实际协作](https://github.com/qinjunw/CozyTown/issues/54)。

### B-005：补充 NPC 周围环境事实与资源条件

- 日期：2026-09-13。
- 用户建议摘要：在有效上下文内提供更详细的 NPC 周围环境信息，减少环境、资源编造和数量判断失误，并核查业界方案。
- 研究发现：已有本人资源数值仍出现错误选择；环境事实、资源可行性、候选字段校验需要分别处理。局部观察、按需查询和行动条件已有一手方案可参考。
- 讨论结果：首轮实验中已知不足时的不可行选择为原版 A 7/15、事实边界指引 B 8/15、B 加自方条件 C 2/15；样本中 C 的错误较少，长期效果未验证。用户随后要求继续，当前固定价玩法采用自方缺口披露、动作过滤及执行复核，已补充 ADR-0018；局部环境观察仍待边界讨论。
- 实验证据：[上下文与资源条件对照实验](verification/npc-grounding-experiment-2026-09-13.md)。160 次固定请求及 16 轮新场景共调用真实模型 217 次；每场景每组仅一次，不作为长期可靠率。
- 研究与实验建议：[NPC 环境事实、资源判断与有效上下文](research/npc-agent/environment-grounding-2026-09-13.md)；[一手资料核查](research/npc-agent/environment-grounding-sources-2026-09-13.md)。
- 关联：AW-FR-006、AW-AC-06、08、09；[调研 NPC 环境事实与行动可行性上下文](https://github.com/qinjunw/CozyTown/issues/64)、[明确资源不足时的邀约与答复策略](https://github.com/qinjunw/CozyTown/issues/62)。

### B-006：采用自方资源条件并保留恢复失败

- 日期：2026-09-13。
- 用户原话：“继续吧”。
- 讨论结果：按地图实施资源不足策略，沿用 v4-flash 和有界真实测试授权。缺钱买方仅等待、缺货卖方仅拒绝，资产变化在提交时重新检查；等待消耗本日机会，暂不增加筹资或条件性承诺。
- 实测反馈：4 种配置各 3 轮，42 次真实调用；资源行为与资产检查符合规则。恢复 11/12，一轮 720 分钟普通工作活动未到期；另有一次过期发言未执行。原失败保留，未补跑。台词环境事实仍未验证。
- 关联：AW-FR-006、011、AW-AC-06、08、09；[实施验证](verification/npc-resource-preconditions-2026-09-13.md)、[ADR-0018](adr/0018-resident-resource-delivery.md)。

### B-007：收束为四人 AI Agent 实验平台

- 日期：2026-09-17。
- 用户确认：“AI Agent 实验平台（推荐）：四人自主运行、完整逻辑快照、可观测和重复实验”。
- 讨论结果：已确认本轮用途与四项能力目标。以[第一版范围与完成条件](wayfinder/agent-world/platform-milestone.md)拆分存档契约、持久化实施、实验入口、集成评测和候选版本交付；不把用途确认当作具体玩法已经决定。
- 保留决策：普通活动期限、精确位置与进行中会面恢复、F／S 实验判定和默认配置。既有失败样本与未合并 PR 状态继续保留。
- 关联：AW-FR-012、014、015、016；AW-AC-02、05、07、09。

### B-008：确认完整逻辑快照的恢复行为

- 日期：2026-09-17。
- 用户原话：“好的,按照推荐来,开始工作吧”。
- 讨论结果：采纳三项推荐，新格式恢复实际位置与路线进度，继续原互动与未完成轮次；旧格式保留已有世界和资产，按明确默认值初始化缺失人物状态并记录迁移来源。
- 已确定契约：[完整逻辑快照 1.0](AGENT_SNAPSHOT_SPEC.md)、[ADR-0019](adr/0019-complete-logical-snapshots-and-agent-continuation.md)。已耗调用、原活动期限、信息权限和资源幂等约束继续有效。
- 实施边界：本轮确认契约，未实现新格式；活动期限和读档重建失败的原生依赖继续保留。关联 AW-FR-012、016，AW-AC-05、07、08。
...
| 文档版本 | 日期 | 变更 |
| --- | --- | --- |
| v0.24 | 2026-09-19 | 重配继承预算、冷却和有效普通待办，真实请求结束前拒绝替换；明确参数收紧与旧会面结束规则，保留完整快照后继范围 |
| v0.25 | 2026-09-19 | 普通自主活动固定原日程窗口截止，每次派发披露剩余时长，超限候选沿用一次纠正；将有效截止与续调约束补入快照契约 |
| v0.23 | 2026-09-17 | 提交后关键重建失败进入暂停恢复状态，隔离显示通知，通过有效重读档恢复；保留真实请求和调用预算 |
| v0.22 | 2026-09-17 | 前置换绑依赖和居民配置校验，补充失败保护、合法重试与普通读档公开测试；完整逻辑快照及提交后重建失败继续按前置依赖推进 |
| v0.21 | 2026-09-17 | 根据用户答复确定快照契约 1.0 与 ADR-0019，将三项恢复选择转为明确验收；保留当前 schema v3 未实施边界与后继依赖 |
| v0.20 | 2026-09-17 | 完整快照盘点补齐小数时刻、路线执行状态、机会消费和逻辑决策进度，形成契约／ADR 草案及 13 组验收；三个恢复选择待确认，运行代码未改 |
| v0.19 | 2026-09-17 | 记录用户确认的实验平台目标，增加完整逻辑快照、四人观察与重复实验的完成条件，拆分后续任务；本轮未改运行代码或新增模型调用 |
...
| v0.4 | 2026-09-12 | 登记两人自主会面、代理实现和首次真实场景结果，记录时间称呼及未确认鱼获问题，保留资源协作和持久化范围 |
| v0.5 | 2026-09-12 | 登记 NPC 独立资产、固定条款原子交付、资产存取、隔离检出及真实模型验证，保留跨日承诺恢复与多次模型评测 |
| v0.6 | 2026-09-13 | 记录多场景资源评测后的环境上下文建议、业界来源与对照实验设计；方案待实验，运行代码未随研究修改 |
| v0.7 | 2026-09-13 | 记录 217 次真实模型上下文实验、16 轮初始化场景和台词盲评；区分资源条件收益、动作过滤和未解决的环境断言，保留后继实施与玩法决策 |
| v0.8 | 2026-09-13 | 采纳资源 v4 自方条件、阶段选项与执行复核；记录 12 轮、42 次真实调用，保留恢复 11/12 和过期发言例外 |
| v0.9 | 2026-09-13 | 增加候选校验与原预算内一次纠正；12 轮、47 次真实调用中 6 次注入错误全部纠正，保留 1 次交付后会面到期；见[本轮报告](verification/npc-candidate-validation-2026-09-13.md) |
| v0.10 | 2026-09-13 | 确认区域＋距离观察、自由台词与结构化事实表达对照、参与者声明边界；增加[专项规格](AGENT_LOCAL_OBSERVATION_SPEC.md)及一手实践核查，运行实现与实验尚待推进 |
| v0.11 | 2026-09-14 | 实现局部观察 v1、参与者来源与时间、请求派发采样和旧事实拒绝；登记场景配置及固定验证，保留后继两组表达实验 |
| v0.12 | 2026-09-14 | 实现有限结构化表达并完成 56 次真实调用；记录 19 个版本拒绝和四世界邀约答复失效，详见[首轮表达对照](verification/npc-expression-comparison-2026-09-14.md) |
| v0.13 | 2026-09-14 | 明确候选行动版本来自请求外层，保持严格拒绝；新固定复测 S 24/24 版本正确，保留选错事实与自由台词问题，实际移动场景恢复待后继验证 |
| v0.14 | 2026-09-14 | 保留已知同区、事实未变的移动邀约答复，并记录执行观察；8 个真实世界完成答复与恢复，详见[移动验证](verification/npc-moving-observation-2026-09-14.md) |
| v0.15 | 2026-09-14 | 复现排队机会过期与预算释放之间的续谈停滞，新增 11 项 Runtime 与 2 项场景对照；生产行为未修复，完整请求重新采样留待[后继任务](https://github.com/qinjunw/CozyTown/issues/83) |
