...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.22，2026-09-17 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 换绑前校验候选时间、资源与角色依赖、居民及动画配置；输入失败保留原世界、会面、活动、时钟和请求资格，并验证合法重试与普通读档对照 |
| 代码参照 | 基于 `96034ae38e116c59409cef8349f47c294608581d`，修改五个生产文件并新增公开入口测试；当前存档仍为 schema v3 |
| 证据方法 | 三条失败路径分别先复现再修复，以固定客户端控制未完成 Task；目标测试、完整回归、独立复核及输入摘要见[换绑修复验证](verification/npc-binding-2026-09-17.md)。真实模型调用为 0；快照契约的 13 组验收仍属后继实现 |
