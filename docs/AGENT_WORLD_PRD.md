...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.18，2026-09-16 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 结合存读档核查 Agent 恢复流程和七项官方来源，明确数据恢复、派生状态重建及旧请求隔离的验收；新增提交后重建失败的独立任务 |
| 代码参照 | 本轮核查冻结于 `85e243edabcead1cfcf8da4d4b90a1633888c49b`；当前存档写入 schema v3，普通读档保留服务引用；生产和测试代码未修改 |
| 证据方法 | 本轮为源码与资料研究，新增 Unity 测试运行和真实模型调用均为 0，见[存读档生命周期研究](research/npc-agent/save-load-lifecycle-2026-09-16.md)。前次 26 项复测见[装配核验](audits/npc-session-composition-2026-09-15.md)，完整回归及四世界 18 次调用见[续谈修复验证](verification/npc-conversation-turn-2026-09-14.md)，均为历史结果 |
