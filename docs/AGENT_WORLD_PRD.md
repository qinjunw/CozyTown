...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.21，2026-09-17 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 按用户确认采纳精确位置与路线续走、进行中互动续接、旧档保留已有数据并初始化缺失状态，确定快照契约 1.0 与 ADR-0019；同步后继实施的验收入口 |
| 代码参照 | 本轮核查冻结于 `7d20bad55a854fb496a8cb295ae93648e66d6141`；当前存档仍为 schema v3；生产和测试代码未修改 |
| 证据方法 | 快照契约依据前次两路源码盘点和本轮用户确认；文档一致性、链接及 GitHub 依赖另行核验，新增运行测试和真实模型调用均为 0，见[快照契约](AGENT_SNAPSHOT_SPEC.md)。前次运行结果见[装配核验](audits/npc-session-composition-2026-09-15.md)及[续谈修复验证](verification/npc-conversation-turn-2026-09-14.md)，均为历史证据 |
