...
| 本轮交付范围 | 完成 A/B/C/D 上下文实验适配器、160 次固定请求与 16 轮双方真实场景，记录资源选择、台词、协议及用量；生产策略采纳由后续票据推进 |
| 代码参照 | 实验代码 `d40fe70b7d59f7e714156471b51fc5f19b86fa85`；历史研究、实施与测试以各阶段报告的独立版本为准 |
| 证据方法 | 一手资料、源码审阅、固定客户端、HTTP 边界及 Unity 测试；新增真实模型调用 217 次，固定探针与场景分开统计，台词经两个 Codex 审阅者独立盲评及分歧核对 |
...
- 讨论结果：首轮实验完成，实施方案待讨论。已知不足时的不可行选择为原版 A 7/15、事实边界指引 B 8/15、B 加自方条件 C 2/15；样本中 C 的错误较少，长期效果未验证。台词仍有无依据事实，动作过滤的 0/15 单列为程序约束效果；尚未采用新的实施 ADR。
