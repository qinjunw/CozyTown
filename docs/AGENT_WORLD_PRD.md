...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.14，2026-09-14 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 已知同区、事实未变时保留移动中的接受／拒绝邀约，并记录执行前重读观察；普通游戏仍默认自由表达 |
| 代码参照 | 移动答复修复冻结于 `80ddc8ae734a818b49685f38e85ac3d635c167c6`；完整输入与结果见[移动验证](verification/npc-moving-observation-2026-09-14.md) |
| 证据方法 | Python 64、隔离 EditMode 828、PlayMode 192 项通过，6 项真实入口默认跳过；另以 v4-flash 运行 8 个新世界、38 次调用，4 次交付交谈和 4 次拒绝均完成，全部恢复日程；候选拒绝与旧请求取消另列 |
