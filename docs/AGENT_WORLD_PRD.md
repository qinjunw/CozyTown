...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.24，2026-09-19 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 同一控制器内重配保留调用历史、角色冷却与有效普通待办；真实请求结束前拒绝替换，校验失败保留原配置 |
| 代码参照 | 基于 `43bc3854f43a98c6b9653bae83a6560e8f14a917`；重配合同见 [ADR-0016](adr/0016-bounded-autonomous-decisions.md)，当前存档仍为 schema v3 |
| 证据方法 | 公开入口控制异步请求完成、失败和重入，验证连续重配、等待期限、活动及资产结果；见[重配验证](verification/npc-reconfiguration-2026-09-19.md)。真实模型调用为 0；完整快照的 13 组验收仍属后继实现 |
...
2026-09-17，用户确认本轮用途为“四人自主运行、完整逻辑快照、可观测和重复实验”的 AI Agent 实验平台。[第一版范围与完成条件](wayfinder/agent-world/platform-milestone.md)规定本轮交付边界：沿用现有地图和已实现行动，补齐逻辑状态恢复、四人实验配置、角色观察与固定输入回放，并在同一候选提交上验收。新增职业生产闭环、需求和长期关系系统留待新的范围决策。用户已进一步采纳精确位置与路线恢复、进行中会面续接及旧档初始化规则；普通活动期限与表达模式判定仍由对应票据确定。
