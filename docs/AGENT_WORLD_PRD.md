...
| 本轮交付范围 | 复现日程变更后等待请求先于预算释放过期的续谈停滞，并登记修复验收；生产行为尚未修复，普通游戏仍默认自由表达 |
| 代码参照 | 生产基线 `ec2082233bf64ff29d86350aaae9e0c0ac77884b`；复现测试 `bb5c4eaf32be6de964b339800b0a1ee79c984f81`；完整输入与结果见[续谈复现](verification/npc-conversation-budget-2026-09-14.md) |
| 证据方法 | 隔离 EditMode 839、PlayMode 194 项通过，6 项真实入口默认跳过；新增真实模型调用 0。保留目标行为失败与现状表征通过的区别；此前 8 个真实世界、38 次调用见[移动验证](verification/npc-moving-observation-2026-09-14.md) |
