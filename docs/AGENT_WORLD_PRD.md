...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.11，2026-09-14 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 区域＋距离观察、对象语义、来源与听众权限、参与者声明、首次派发采样和执行前复核；两组表达对照尚待后继任务 |
| 代码参照 | 从观察设计提交 `cd2daae033b0311200637922156279c9224f8ca1` 实施；代码输入和结果见[局部观察验证](verification/npc-local-observation-2026-09-14.md) |
| 证据方法 | 普通 C#、Unity 身体、真实开发场景和 JSON／代理固定回归；本轮无真实模型调用，固定投影通过不等于台词事实正确 |
| 未验证内容 | 完整局部环境事实的效果、长期可靠率、四人共同负载、长期角色差异及跨日持久化稳定性 |
