...
| 字段 | 值 |
| --- | --- |
| 文档版本 | v0.25，2026-09-19 |
| 状态 | 世界事件、有界决策、两人自主会面和资源交付已接入；会面承诺与经历持久化、四人多日评测继续按地图迭代 |
| 用途 | 定义 Agent 世界的目标、需求、验收、实施依赖与研究依据，作为后续任务拆分和设计讨论的入口 |
| 本轮交付范围 | 普通自主活动在当前日程窗口内结束；逐次披露剩余时长，超限候选沿用一次有界纠正，迟到的合法回复不延长窗口 |
| 代码参照 | 基于 `bca2a6c272cead7a1ea0f303a49cf39addc60c0c`；活动与决策合同见 [ADR-0015](adr/0015-resident-activity-arbitration-and-world-epochs.md)和 [ADR-0016](adr/0016-bounded-autonomous-decisions.md)，当前存档仍为 schema v3 |
| 证据方法 | 公开入口控制回复时刻、候选时长、预算及世界推进，分别检查活动释放和身体恢复；见[活动窗口验证](verification/npc-activity-window-2026-09-19.md)。真实模型调用为 0；完整快照的 13 组验收仍属后继实现 |
| 未验证内容 | 长期可靠率、四人共同负载、长期角色差异及跨日持久化稳定性；台词事实正确性和表达质量不能由流程完成证明 |
